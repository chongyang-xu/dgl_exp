...
parser.add_argument('--dataset', type=str, required=True, choices=['ogbpr', 'ogbar', 'ogbpa', 'cora', 'reddit'], help='')
...
FIRST_N= f"{args.use_first_n_parts}" if args.use_first_n_parts > 0 else N_PARTS

IP_CONF="/data/ipconfigs/{}.txt".format(args.ip_config)
...
TRAIN_LAUNCHER="python3 /workspace/dgl_dsg/tools/launch.py --workspace /workspace/dgl_dsg/experiments"
PYTHON_CMD_PATH="python3"

SINGLE_JOB_CMD = "{PYTHON_CMD_PATH} models/train_from_chunks.py".format(PYTHON_CMD_PATH=PYTHON_CMD_PATH)
...
#                    DS_NAME=args.dataset
#                )
SINGLE_JOB_CMD += f" --graph_name {args.dataset} --n_parts {N_PARTS}"

SINGLE_JOB_CMD += " --num_gpus {N_GPUS} --ip_config {IP_CONF}".format(
...
LAUNCH_JOBS_CMD_PREFIX += " --part_conf {PART_CONF} --ip_config {IP_CONF}".format(
                        #PART_CONF="/data/ds_pre/ogbpr/data_part_n64_metis0_1_64/ogbpr.json",
                        PART_CONF=f"/data/ds_pre/{args.dataset}/data_part_n{N_PARTS}_metis_1_{N_PARTS}/{args.dataset}.json",
                        IP_CONF=IP_CONF,
                    )
if args.verbose:
