...
f" --model {MODEL} --layers {NUM_LAYER} --one_id sampledbg_trainctrl_{DATA_SET}_{NUM_PART}_{first_n}_{extra_tag}_{MODEL}_{NUM_LAYER}_bdr" \
...
f" --model {MODEL} --layers {NUM_LAYER} --one_id ppt_train_{DATA_SET}_{NUM_PART}_{first_n}_{PART_ALGO}_{MODEL}_{NUM_LAYER}_{sampling_strategy}" \
f" --batch_size {BATCH_SIZE} --batch_size_eval {EVAL_BATCH_SIZE} --eval_every {N_EVAL} --n_epoch {N_EPOCH}" \
f" --n_mach {n_mach} --n_gpu_per_mach 1 --n_server_per_mach {n_svr_per_mach} --n_trainer_per_mach {n_trainer_per_mach} --n_sampler_per_trainer 0" \
f" --disable_backup_server True --ip_config {ip_config}" \
...
run_training([ 'ogbpr' ], [4], -1, ['metis'], [3], ['sage'], extra_tag="default", num_halo=1, sampling_strategy="dft")
exit(0)
