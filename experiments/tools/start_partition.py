...
#            for PART_ALGO in [ 'random', 'metis', 'vcrandom', 'vcoblivious', 'vcdeg', 'vcns']:
                for ALGO_TAG in range(1):
                    ...
                    for NUM_HOP in [ halo_hop ]:
                        ...
                        CMD=f"python3 /workspace/dgl_dsg/experiments/tools/preprocessing.py  --dataset {DATA_SET} --part-algo {PART_ALGO} --algo-tag {ALGO_TAG} --num-hops {NUM_HOP} --n-parts {NUM_PARTS} --one_id {RUN_ID}  --data-root-path {DATA_ROOT} --save-first-n-parts {save_first_n}"
                        os.system(CMD)
...
run(['ogbpr'], [4, 16], save_first_n=-1, halo_hop=0)
#run(['ogbpr'], [256, 1024], save_first_n=64)
