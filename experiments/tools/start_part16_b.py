...
extra_tag = "random_xno_part_bias_050x"
for idx in range(3):
    run_training_ctrl(['reddit', 'cora',], [16], -1, None, [3], ['gcn', 'sage', 'gat'], extra_tag=extra_tag, switch_mode=3)
exit(0)

for idx in range(3):
    run_training_ctrl(['reddit', 'ogbpr', 'ogbar' ], [16], -1, None, [2], ['gcn'], extra_tag=extra_tag, switch_mode=3)
    run_training_ctrl(['reddit', 'ogbpr', 'ogbar' ], [16], -1, None, [2], ['sage'], extra_tag=extra_tag, switch_mode=3)
    run_training_ctrl(['reddit', 'ogbpr', 'ogbar' ], [16], -1, None, [2], ['gat'], extra_tag=extra_tag, switch_mode=3)

for idx in range(3):
    run_training_ctrl(['reddit', 'ogbpr', 'ogbar' ], [16], -1, None, [3], ['gcn'], extra_tag=extra_tag, switch_mode=3)
    run_training_ctrl(['reddit', 'ogbpr', 'ogbar' ], [16], -1, None, [3], ['sage'], extra_tag=extra_tag, switch_mode=3)
    run_training_ctrl(['reddit', 'ogbpr', 'ogbar' ], [16], -1, None, [3], ['gat'], extra_tag=extra_tag, switch_mode=3)

exit(0)

#exit(0)
run_training(['reddit', 'ogbpr'], [16], -1, ['metis'], [4], ['sage'], extra_tag="default", num_halo=1, sampling_strategy="dft")
exit(0)

for dataset in ['ogbpa']:
    for i in range(3):
        extra_tag = "mode0_random_xno_part_bias_500x"
        run_training_ctrl([dataset], [16], -1, None, [2], ['gcn'], extra_tag=extra_tag, switch_mode=3)
        run_training_ctrl([dataset], [16], -1, None, [2], ['sage'], extra_tag=extra_tag, switch_mode=3)
        run_training_ctrl([dataset], [16], -1, None, [2], ['gat'], extra_tag=extra_tag, switch_mode=3)
